...
#define HOST_NUM 4 // the number of hosts per switch
#define PORT 5 // the number of ports per switch
...
struct Pair {
   ...
   bool Valid;
   // 初期化
   Pair(int s, int d): src(s),dst(d),ID(-1),Valid(false){}
};
...
int max_id, vector<Pair> pairs, int hops)
{
   cout << endl;
   // 各チャネル毎に通過する経路の ID を表示、およびエラーチェック
   for (int i=0; i < PORT*(node_num+HOST_NUM+1); i++){
   ...
   cout << "(Maximum) Crossing Paths: " 
       << max_element(Crossing_Paths.begin(),Crossing_Paths.end())->pair_index.size() << endl;
...
int main(int argc, char *argv[])
{
   ...
   vector<Cross_Paths> Crossing_Paths( PORT * (node_num+PORT) ); // port num * switch num
   // 経路数を格納
   int ct = 0;
   // 全経路の総ホップ数
   int hops = 0; 
   ...
   while ( cin >> src){	
	 ...
	 int t;
	 // root switch
	 if ( current == node_num + HOST_NUM ){
	    t = current * PORT + dst/HOST_NUM+1;
	    current = current - HOST_NUM + dst/HOST_NUM;
	 // middle layer switch
	 } else if ( current >= node_num){
	    if ( current-node_num != dst/HOST_NUM){
	       t = current * PORT + 0;
	       current = node_num + HOST_NUM;
	    } else {
	       t = current * PORT + dst%HOST_NUM+1;
	       current = (current%HOST_NUM)*HOST_NUM + dst%HOST_NUM;
	    }
	 // host switch
	 } else {
	    // 不要
   ...
   for (int j = 0; j < PORT * (node_num+HOST_NUM+1); j++ ){ 
      // 経路を選ぶためのチャネルを選択。
   ...
   show_paths(Crossing_Paths, ct, node_num, max_id, pairs, hops);   
