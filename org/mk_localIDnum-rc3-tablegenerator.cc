...
bool path_based, int array_size, vector<int> Output) //huyao170217
{
   cout << endl;
...
//	 cout << " channel from " << pairs[t].src << " to " << pairs[t].dst << " is assigned to the ID " << pairs[t].ID << endl;
   ...
   if (Vch == 0)
		...
		<< endl;
   int node = port/((degree+1+2*Host_Num)*Vch); //huyao 170217
      ...
      //huyao 170217
      int east = (node+1)%array_size==0 ? node+1-array_size : node+1;
      int west = node%array_size==0 ? node-1+array_size : node-1;
      int south = (node+array_size)%(array_size*array_size);
      int north = (node-array_size+array_size*array_size)%(array_size*array_size);
      if (pointer%(degree+1+2*Host_Num) == 1) Output[east*degree] += (*elem).pair_index.size();
      if (pointer%(degree+1+2*Host_Num) == 2) Output[west*degree+1] += (*elem).pair_index.size();
      if (pointer%(degree+1+2*Host_Num) == 3) Output[south*degree+2] += (*elem).pair_index.size();
      if (pointer%(degree+1+2*Host_Num) == 4) Output[north*degree+3] += (*elem).pair_index.size();

      if (pointer<degree && slots<(*elem).pair_index.size())  slots = (*elem).pair_index.size(); //huyao161226
      elem ++; port++; pointer++;
...
//   cout << "(Maximum) Crossing Paths: " << max_cp << endl;
   ...
   cout << "(Maximum) number of slots: " << slots << endl;

   //huyao 170217
   cout << "" << endl;
   for (int i=0; i<degree*switch_num; i++)
	{
		if (i%degree==0)
		{
			cout << endl;
			cout << " Node " << setw(2) << i/degree << ":  ";
		}	
		cout << " " << Output[i] ;
	}
...
int main(int argc, char *argv[])
{
   ...
   vector<int> Output(degree*switch_num); //huyao 170217
   for (int i=0; i<degree*switch_num; i++) {Output[i]=0;} //huyao 170217
   // 経路数を格納
   int ct = 0;
   // 総経路の総ホップ数
   int hops = 0; 
...
array_size, Output);  //huyao 170217 
