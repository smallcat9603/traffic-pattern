...
int main(int argc, char *argv[])
{
   ...
   vector<Cross_Paths> Crossing_Paths_xyz(Crossing_Paths.size());
   vector<Cross_Paths> Crossing_Paths_xzy(Crossing_Paths.size());   
   vector<Cross_Paths> Crossing_Paths_yxz(Crossing_Paths.size());
   vector<Cross_Paths> Crossing_Paths_yzx(Crossing_Paths.size());   
   vector<Cross_Paths> Crossing_Paths_zxy(Crossing_Paths.size());
   vector<Cross_Paths> Crossing_Paths_zyx(Crossing_Paths.size());   
   vector<Pair> pairs_xyz;
   vector<Pair> pairs_xzy;  
   vector<Pair> pairs_yxz;
   vector<Pair> pairs_yzx;  
   vector<Pair> pairs_zxy;
   vector<Pair> pairs_zyx;       
   ...
   while ( cin >> h_src){	
      ...
      int xx, yy, zz, xx_sd, yy_sd, zz_sd;
      int src_sd, dst_sd;
      src_sd = h_src/Host_Num;
      dst_sd = h_dst/Host_Num;
      switch (Topology){
      case 0: //mesh
	  zz = dst/(array_size*array_size) - src/(array_size*array_size); //huyao 170101
	  xx = (dst%(array_size*array_size))%array_size - (src%(array_size*array_size))%array_size; //huyao 170101
	  yy = (dst%(array_size*array_size))/array_size - (src%(array_size*array_size))/array_size; //huyao 170101
      xx_sd = (dst%(array_size*array_size))%array_size - (src%(array_size*array_size))%array_size; //huyao180410
      yy_sd = (dst%(array_size*array_size))/array_size - (src%(array_size*array_size))/array_size; //huyao180410
      zz_sd = dst/(array_size*array_size) - src/(array_size*array_size); //huyao180410
	  break;

      case 1: // torus
	  //delta_z huyao 170102
	  zz = dst/(array_size*array_size) - src/(array_size*array_size);
      zz_sd = dst/(array_size*array_size) - src/(array_size*array_size); //huyao180410
	  if ( zz < 0 && abs(zz) > array_size/2 ) {
	    zz = -( zz + array_size/2);
        zz_sd = -( zz_sd + array_size/2); //huyao180410
	  } else if ( zz > 0 && abs(zz) > array_size/2 ) {
	    zz = -( zz - array_size/2);
        zz_sd = -( zz_sd - array_size/2); //huyao180410
	  }
	  xx = (dst%(array_size*array_size))%array_size - (src%(array_size*array_size))%array_size; //huyao 170101
      xx_sd = (dst%(array_size*array_size))%array_size - (src%(array_size*array_size))%array_size; //huyao 180410
	  if ( xx < 0 && abs(xx) > array_size/2 ) {
	    xx = -( xx + array_size/2);
        xx_sd = -( xx_sd + array_size/2); //huyao 180410	
	  } else if ( xx > 0 && abs(xx) > array_size/2 ) {
	    xx = -( xx - array_size/2);
        xx_sd = -( xx_sd - array_size/2); //huyao180410	
	  }
	    yy = (dst%(array_size*array_size))/array_size - (src%(array_size*array_size))/array_size; //huyao 170101
        yy_sd = (dst%(array_size*array_size))/array_size - (src%(array_size*array_size))/array_size; //huyao 180410
	  if ( yy < 0 && abs(yy) > array_size/2 ) {
	    yy = -( yy + array_size/2);
        yy_sd = -( yy_sd + array_size/2); //huyao 180410	
      ...
      if (abs(xx_sd) < 2 || abs(yy_sd) < 2 || abs(zz_sd) < 2){        
         obli -= 2;
      } else{
         obli -= 1;
         if (obli == 1){
      ...
      int delta_x_sd, delta_y_sd, delta_z_sd; //huyao180410
      switch (Topology){
      case 0: //mesh
	 ...
	 delta_y = dst_xy/array_size - src_xy/array_size; //huyao 170101
         ...
         delta_z_sd = dst/(array_size*array_size) - src/(array_size*array_size); //huyao180410
	 current = src; 
	 break;
	 ...
	 if ( delta_z < 0 && abs(delta_z) > array_size/2 ) {
	    delta_z = -( delta_z + array_size/2);
            delta_z_sd = -( delta_z_sd + array_size/2); //huyao180410
		wrap_around_z = true;	
                wrap_around_z_sd = true; //huyao180410	
	 ...
	 if ( delta_x < 0 && abs(delta_x) > array_size/2 ) {
	    delta_x = -( delta_x + array_size/2);
            delta_x_sd = -( delta_x_sd + array_size/2); //huyao 180410
		wrap_around_x = true;	
                wrap_around_x_sd = true; //huyao 180410		
	 ...
	 if ( delta_y < 0 && abs(delta_y) > array_size/2 ) {
	    delta_y = -( delta_y + array_size/2);
            delta_y_sd = -( delta_y_sd + array_size/2); //huyao 180410
		wrap_around_y = true;
                wrap_around_y_sd = true; //huyao 180410		
	 ...
	 pairs[ct].hops = abs(delta_x) + abs(delta_y) + abs(delta_z); //huyao 170102
      ...
      hops += abs(delta_x) + abs(delta_y) + abs(delta_z);
      copy(Crossing_Paths.begin(), Crossing_Paths.end(), Crossing_Paths_xyz.begin());
      copy(Crossing_Paths.begin(), Crossing_Paths.end(), Crossing_Paths_xzy.begin());
      copy(Crossing_Paths.begin(), Crossing_Paths.end(), Crossing_Paths_yxz.begin());
      copy(Crossing_Paths.begin(), Crossing_Paths.end(), Crossing_Paths_yzx.begin());
      copy(Crossing_Paths.begin(), Crossing_Paths.end(), Crossing_Paths_zxy.begin());
      copy(Crossing_Paths.begin(), Crossing_Paths.end(), Crossing_Paths_zyx.begin());
      pairs_xyz = pairs;
      pairs_xzy = pairs;  
      pairs_yxz = pairs;
      pairs_yzx = pairs; 
	 ...
	 while ( delta_z != 0 ){ // +x方向
	    ...
	    Crossing_Paths_zxy[t].pair_index.push_back(ct); //huyao180410
	    pairs_zxy[ct].channels.push_back(t); //huyao180410
...
//       }
	 ...
	 while ( delta_x != 0 ){ // +x方向
	    int t = (wrap_around_x) ? Vch*current*(degree+1+2*Host_Num)+1+(degree+1+2*Host_Num) :
	       Vch * current * (degree+1+2*Host_Num) + 1;
	    Crossing_Paths_xyz[t].pair_index.push_back(ct); //huyao180410
	    pairs_xyz[ct].channels.push_back(t); //huyao180410
	    if ( (current % (array_size*array_size)) % array_size == array_size-1) { //huyao 170102
	       wrap_around_x = false;
	       current = current - (array_size -1);
	    } else current++; 
	 ...
	 while ( delta_x != 0 ){ // +x方向
	    int t = (wrap_around_x) ? Vch*current*(degree+1+2*Host_Num)+1+(degree+1+2*Host_Num) :
	       Vch * current * (degree+1+2*Host_Num) + 1;
	    Crossing_Paths_xzy[t].pair_index.push_back(ct); //huyao180410
	    pairs_xzy[ct].channels.push_back(t); //huyao180410
	    if ( (current % (array_size*array_size)) % array_size == array_size-1) { //huyao 170102
	       wrap_around_x = false;
	       current = current - (array_size -1);
	    } else current++; 
	 ...
	 while ( delta_y != 0 ){ // +y方向
	    int t = (wrap_around_y) ? Vch*current*(degree+1+2*Host_Num)+3+(degree+1+2*Host_Num) :
	       Vch * current * (degree+1+2*Host_Num) + 3;
	    Crossing_Paths_yxz[t].pair_index.push_back(ct); //huyao180410
	    pairs_yxz[ct].channels.push_back(t); //huyao180410
	    if ( (current % (array_size*array_size)) >= array_size*(array_size-1) ){ //huyao 170102
	       wrap_around_y = false;
	       current = current - array_size*(array_size -1);
	    } else current += array_size;
	 ...
	 while ( delta_y != 0 ){ // +y方向
	    int t = (wrap_around_y) ? Vch*current*(degree+1+2*Host_Num)+3+(degree+1+2*Host_Num) :
	       Vch * current * (degree+1+2*Host_Num) + 3;
	    Crossing_Paths_yzx[t].pair_index.push_back(ct); //huyao180410
	    pairs_yzx[ct].channels.push_back(t); //huyao180410
	    if ( (current % (array_size*array_size)) >= array_size*(array_size-1) ){ //huyao 170102
	       wrap_around_y = false;
	       current = current - array_size*(array_size -1);
	    } else current += array_size;
	 ...
	 while ( delta_z != 0 ){ // +x方向
	    int t = (wrap_around_z) ? Vch*current*(degree+1+2*Host_Num)+5+(degree+1+2*Host_Num) :
	       Vch * current * (degree+1+2*Host_Num) + 5;
	    Crossing_Paths_zyx[t].pair_index.push_back(ct); //huyao180410
	    pairs_zyx[ct].channels.push_back(t); //huyao180410
	    //if ( current % (array_size*array_size) == array_size-1) { //huyao 170103
	    if ( current >= array_size*array_size*(array_size-1)) {
	       wrap_around_z = false;
	       current = current - (array_size -1)*array_size*array_size;
	 ...
	 exit (1);
      ...
      int slots_xyz = max_element(Crossing_Paths_xyz.begin(),Crossing_Paths_xyz.end())->pair_index.size();
      int slots_xzy = max_element(Crossing_Paths_xzy.begin(),Crossing_Paths_xzy.end())->pair_index.size();
      int slots_yxz = max_element(Crossing_Paths_yxz.begin(),Crossing_Paths_yxz.end())->pair_index.size();
      int slots_yzx = max_element(Crossing_Paths_yzx.begin(),Crossing_Paths_yzx.end())->pair_index.size();
      int slots_zxy = max_element(Crossing_Paths_zxy.begin(),Crossing_Paths_zxy.end())->pair_index.size();
      int slots_zyx = max_element(Crossing_Paths_zyx.begin(),Crossing_Paths_zyx.end())->pair_index.size();
      int slots[] = {slots_xyz, slots_xzy, slots_yxz, slots_yzx, slots_zxy, slots_zyx};
      int slots_min = *min_element(slots, slots+6);
      if (slots_xyz == slots_min){
         copy(Crossing_Paths_xyz.begin(), Crossing_Paths_xyz.end(), Crossing_Paths.begin()); 
         pairs = pairs_xyz;
      } else if (slots_xzy == slots_min){
         copy(Crossing_Paths_xzy.begin(), Crossing_Paths_xzy.end(), Crossing_Paths.begin());  
         pairs = pairs_xzy;
      } else if (slots_yxz == slots_min){
         copy(Crossing_Paths_yxz.begin(), Crossing_Paths_yxz.end(), Crossing_Paths.begin());  
         pairs = pairs_yxz;
      } else if (slots_yzx == slots_min){
         copy(Crossing_Paths_yzx.begin(), Crossing_Paths_yzx.end(), Crossing_Paths.begin());  
         pairs = pairs_yzx;
      } else if (slots_zxy == slots_min){
         copy(Crossing_Paths_zxy.begin(), Crossing_Paths_zxy.end(), Crossing_Paths.begin());  
         pairs = pairs_zxy;
      } else if (slots_zyx == slots_min){
         copy(Crossing_Paths_zyx.begin(), Crossing_Paths_zyx.end(), Crossing_Paths.begin());  
         pairs = pairs_zyx;
      }  
