...
int max_id, vector<Pair> pairs, int hops, int max_cp)
{
   ...
   if (HOST_NUM == 4) // switch_num = 1+4+16=21
   {
	   cout << "avg_slots: " << (float)total_slots/(21*PORT) << endl;
	   cout << "unused_total_slots: " << 21*PORT*max_cp-total_slots << endl;
	   cout << "unused_avg_slots: " << (float)(21*PORT*max_cp-total_slots)/(21*PORT) << endl;
   }
   if (HOST_NUM == 16) // switch_num = 1+16+256=273
   {
	   cout << "avg_slots: " << (float)total_slots/(273*PORT) << endl;
	   cout << "unused_total_slots: " << 273*PORT*max_cp-total_slots << endl;
	   cout << "unused_avg_slots: " << (float)(273*PORT*max_cp-total_slots)/(273*PORT) << endl;
   }
