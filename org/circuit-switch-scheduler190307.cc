...
struct Pair
{
        ...
        int hops;
        // initialize
        Pair(int s, int d, int h_s, int h_d) : src(s), dst(d), h_src(h_s), h_dst(h_d), flow_id(-1), job_id(-1), ID(-1), Valid(false), hops(-1) {}
                   ...
                   bool path_based, int degree, vector<int> Switch_Topo, vector<int> topo_sws_uni, int default_slot)
{
        ...
        for (int i = 0; i < pairs.size(); i++)
        {
                ...
                if (src_index == -1 || dst_index == -1)
                        outputfile << "Error: src/dst number is wrong" << endl;
                ...
                { //current_pair.channels[0] --> src, current_pair.channels[current_pair.channels.size()-1] --> dst
                        ...
                        else
                        {
                                ...
                                //cout << "SW " << target_sw << " (port " << output_port << ")" << " --> ";
                                outputfile << "SW " << topo_sws_uni[target_sw] << " (port " << input_port << "->" << output_port << ")"
                                           << " - [slot " << slot_num << "] -> ";
                        }
                        //     char filename[100];
                        //     sprintf(filename, "output/sw%d", target_sw); // save to output/
        ...
        for (int i = 1; i < all_jobs.size(); i++)
        {
                ...
                if (submit_interval > 0)
                {
                        ...
                        if (queue.size() > 2)
                        {
                                ...
                                else if (queue_policy == 2)
                                { //sf
                                        sort(queue.begin() + 1, queue.end(), SSF);
                                }
                                else if (queue_policy == 3)
                                { //rlf
                                        sort(queue.begin() + 1, queue.end(), LRF);
                                }
                                else if (queue_policy == 4)
                                { //rsf
        ...
        for (int p = 0; p < job.src_dst_pairs_m.size(); p++)
        {
                ...
                for (int i = 0; i < job.src_dst_pairs_m[p].channels.size(); i++)
                {
                        Crossing_Paths[job.src_dst_pairs_m[p].channels[i]].pair_index.erase(remove(Crossing_Paths[job.src_dst_pairs_m[p].channels[i]].pair_index.begin(), Crossing_Paths[job.src_dst_pairs_m[p].channels[i]].pair_index.end(),
                                                                                                   job.src_dst_pairs_m[p].pair_id),
                                                                                            Crossing_Paths[job.src_dst_pairs_m[p].channels[i]].pair_index.end());
                        Crossing_Paths[job.src_dst_pairs_m[p].channels[i]].flow_index.erase(remove(Crossing_Paths[job.src_dst_pairs_m[p].channels[i]].flow_index.begin(), Crossing_Paths[job.src_dst_pairs_m[p].channels[i]].flow_index.end(),
                                                                                                   job.job_id * 1000 + job.src_dst_pairs_m[p].flow_id),
                                                                                            Crossing_Paths[job.src_dst_pairs_m[p].channels[i]].flow_index.end());
                        Crossing_Paths[job.src_dst_pairs_m[p].channels[i]].assigned_list.erase(remove(Crossing_Paths[job.src_dst_pairs_m[p].channels[i]].assigned_list.begin(), Crossing_Paths[job.src_dst_pairs_m[p].channels[i]].assigned_list.end(),
                                                                                                      job.flows[job.src_dst_pairs_m[p].flow_id].ID),
                                                                                               Crossing_Paths[job.src_dst_pairs_m[p].channels[i]].assigned_list.end());
                        Crossing_Paths[job.src_dst_pairs_m[p].channels[i]].assigned_dst_list.erase(remove(Crossing_Paths[job.src_dst_pairs_m[p].channels[i]].assigned_dst_list.begin(), Crossing_Paths[job.src_dst_pairs_m[p].channels[i]].assigned_dst_list.end(),
                                                                                                          job.src_dst_pairs_m[p].h_dst),
                                                                                                   Crossing_Paths[job.src_dst_pairs_m[p].channels[i]].assigned_dst_list.end());
